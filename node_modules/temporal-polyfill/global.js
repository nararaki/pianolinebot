!function() {
  ...
  function clampRelativeDuration(durationFields, clampUnit, clampDistance, marker, markerToEpochNano, moveMarker) {
    const clampDurationFields = {
      ...durationFieldDefaults,
      [durationFieldNamesAsc[clampUnit]]: clampDistance
    }, marker0 = moveMarker(marker, durationFields), marker1 = moveMarker(marker0, clampDurationFields);
  ...
  function roundDayTimeDurationByInc(durationFields, nanoInc, roundingMode) {
    const maxUnit = Math.min(getLargestDurationUnit(durationFields), 6);
  ...
  function roundRelativeDuration(durationFields, endEpochNano, largestUnit, smallestUnit, roundingInc, roundingMode, marker, markerToEpochNano, moveMarker) {
    if (0 === smallestUnit && 1 === roundingInc) {
      return durationFields;
    }
    const nudgeFunc = smallestUnit > 6 ? nudgeRelativeDuration : isZonedEpochSlots(marker) && smallestUnit < 6 ? nudgeZonedTimeDuration : nudgeDayTimeDuration;
    let [roundedDurationFields, roundedEpochNano, grewBigUnit] = nudgeFunc(durationFields, endEpochNano, largestUnit, smallestUnit, roundingInc, roundingMode, marker, markerToEpochNano, moveMarker);
    return grewBigUnit && (roundedDurationFields = ((durationFields, endEpochNano, largestUnit, smallestUnit, marker, markerToEpochNano, moveMarker) => {
      ...
      for (let currentUnit = smallestUnit + 1; currentUnit <= largestUnit; currentUnit++) {
        ...
        const beyondThreshold = bigNanoToNumber(diffBigNanos(markerToEpochNano(moveMarker(marker, baseDurationFields)), endEpochNano));
      ...
      return durationFields;
    })(roundedDurationFields, roundedEpochNano, largestUnit, Math.max(6, smallestUnit), marker, markerToEpochNano, moveMarker)), 
  ...
  function nudgeZonedTimeDuration(durationFields, endEpochNano, _largestUnit, smallestUnit, roundingInc, roundingMode, marker, markerToEpochNano, moveMarker) {
    const sign = computeDurationSign(durationFields);
    let [dayDelta, timeNano] = durationFieldsToBigNano(durationFields, 5);
    ...
    }, 6, sign, marker, markerToEpochNano, moveMarker), beyondDay = roundedTimeNano - bigNanoToNumber(diffBigNanos(dayEpochNano0, dayEpochNano1));
    beyondDay && Math.sign(beyondDay) !== sign ? endEpochNano = moveBigNano(dayEpochNano0, roundedTimeNano) : (dayDelta += sign, 
    roundedTimeNano = roundByInc(beyondDay, nanoInc, roundingMode), endEpochNano = moveBigNano(dayEpochNano1, roundedTimeNano));
  ...
  function nudgeRelativeDuration(durationFields, endEpochNano, _largestUnit, smallestUnit, roundingInc, roundingMode, marker, markerToEpochNano, moveMarker) {
    const sign = computeDurationSign(durationFields), smallestUnitFieldName = durationFieldNamesAsc[smallestUnit], baseDurationFields = clearDurationFields(smallestUnit, durationFields), truncedVal = divTrunc(durationFields[smallestUnitFieldName], roundingInc) * roundingInc;
    baseDurationFields[smallestUnitFieldName] = truncedVal;
    const [epochNano0, epochNano1] = clampRelativeDuration(baseDurationFields, smallestUnit, roundingInc * sign, marker, markerToEpochNano, moveMarker), exactVal = truncedVal + computeEpochNanoFrac(endEpochNano, epochNano0, epochNano1) * sign * roundingInc, roundedVal = roundByInc(exactVal, roundingInc, roundingMode), expanded = Math.sign(roundedVal - exactVal) === sign;
  ...
  function moveZonedDateTime(getCalendarOps, getTimeZoneOps, doSubtract, zonedDateTimeSlots, durationSlots, options = Object.create(null)) {
    ...
    return {
      ...zonedDateTimeSlots,
      ...moveZonedEpochs(calendarOps, timeZoneOps, zonedDateTimeSlots, doSubtract ? negateDurationFields(durationSlots) : durationSlots, options)
    };
  }
  ...
  function moveZonedEpochs(calendarOps, timeZoneOps, slots, durationFields, options) {
  ...
  function diffZonedDateTimes(getCalendarOps, getTimeZoneOps, invert, slots0, slots1, options) {
    ...
    if (sign) {
      if (largestUnit < 6) {
        ...
        durationFields = roundRelativeDuration(durationFields, epochNano1, largestUnit, smallestUnit, roundingInc, roundingMode, slots0, extractEpochNano, bindArgs(moveZonedEpochs, calendarOps, timeZoneOps));
      }
    } else {
  ...
  function diffPlainDateTimes(getCalendarOps, invert, plainDateTimeSlots0, plainDateTimeSlots1, options) {
    ...
    if (sign) {
      if (largestUnit <= 6) {
        ...
        durationFields = roundRelativeDuration(durationFields, endEpochNano, largestUnit, smallestUnit, roundingInc, roundingMode, plainDateTimeSlots0, isoToEpochNano, bindArgs(moveDateTime, calendarOps));
      }
    } else {
      durationFields = durationFieldDefaults;
    }
  ...
  function diffDateLike(invert, getCalendarOps, startIsoFields, endIsoFields, largestUnit, smallestUnit, roundingInc, roundingMode, origOptions) {
    ...
    if (compareBigNanos(endEpochNano, startEpochNano)) {
      if (6 === largestUnit) {
        ...
        6 === smallestUnit && 1 === roundingInc || (durationFields = roundRelativeDuration(durationFields, endEpochNano, largestUnit, smallestUnit, roundingInc, roundingMode, startIsoFields, isoToEpochNano, bindArgs(moveDate, calendarOps)));
      }
    } else {
      durationFields = durationFieldDefaults;
    }
  ...
  function diffZonedEpochsExact(calendarOps, timeZoneOps, slots0, slots1, largestUnit, origOptions) {
  ...
  function diffDateTimesBig(calendarOps, startIsoFields, endIsoFields, sign, largestUnit, origOptions) {
    let midIsoFields = startIsoFields, timeDiffNano = diffTimes(startIsoFields, endIsoFields);
    return Math.sign(timeDiffNano) === -sign && (midIsoFields = moveByDays(startIsoFields, sign), 
    timeDiffNano += nanoInUtcDay * sign), {
      ...calendarOps.dateUntil(midIsoFields, endIsoFields, largestUnit, origOptions),
  ...
  function createMoveMarker(calendarOps, timeZoneOps) {
    return timeZoneOps ? bindArgs(moveZonedEpochs, calendarOps, timeZoneOps) : bindArgs(moveDateTime, calendarOps);
  }
  function createDiffMarkers(calendarOps, timeZoneOps) {
    return timeZoneOps ? bindArgs(diffZonedEpochsExact, calendarOps, timeZoneOps) : bindArgs(diffDateTimesExact, calendarOps);
  }
  ...
  function spanDuration(durationFields, largestUnit, marker, markerToEpochNano, moveMarker, diffMarkers) {
    const endMarker = moveMarker(marker, durationFields);
    return [ diffMarkers(marker, endMarker, largestUnit), markerToEpochNano(endMarker) ];
  }
  function addDurations(refineRelativeTo, getCalendarOps, getTimeZoneOps, doSubtract, slots, otherSlots, options) {
    const relativeToSlots = refineRelativeTo(normalizeOptions(options).relativeTo), maxUnit = Math.max(getLargestDurationUnit(slots), getLargestDurationUnit(otherSlots));
    ...
    const [marker, calendarOps, timeZoneOps] = createMarkerSystem(getCalendarOps, getTimeZoneOps, relativeToSlots), moveMarker = createMoveMarker(calendarOps, timeZoneOps), diffMarkers = createDiffMarkers(calendarOps, timeZoneOps), midMarker = moveMarker(marker, slots);
    return createDurationSlots(diffMarkers(marker, moveMarker(midMarker, otherSlots), maxUnit));
  }
  ...
  const timeZoneNameStrs = [ "timeZoneName" ], monthDayFallbacks = {
    ...
    round: (slots, options) => createDuration(((refineRelativeTo, getCalendarOps, getTimeZoneOps, slots, options) => {
      ...
      if (isUniformUnit(Math.max(durationLargestUnit, largestUnit), relativeToSlots)) {
      ...
      const [marker, calendarOps, timeZoneOps] = createMarkerSystem(getCalendarOps, getTimeZoneOps, relativeToSlots), markerToEpochNano = createMarkerToEpochNano(timeZoneOps), moveMarker = createMoveMarker(calendarOps, timeZoneOps), diffMarkers = createDiffMarkers(calendarOps, timeZoneOps);
      let transplantedWeeks = 0;
      slots.weeks && 7 === smallestUnit && (transplantedWeeks = slots.weeks, slots = {
        ...slots,
        weeks: 0
      });
      let [balancedDuration, endEpochNano] = spanDuration(slots, largestUnit, marker, markerToEpochNano, moveMarker, diffMarkers);
      ...
      return balancedSign && (balancedDuration = roundRelativeDuration(balancedDuration, endEpochNano, largestUnit, smallestUnit, roundingInc, roundingMode, marker, markerToEpochNano, moveMarker)), 
    ...
    total: (slots, options) => ((refineRelativeTo, getCalendarOps, getTimeZoneOps, slots, options) => {
      ...
      if (isUniformUnit(Math.max(totalUnit, durationLargestUnit), relativeToSlots)) {
        return ((durationFields, totalUnit) => bigNanoToNumber(durationFieldsToBigNano(durationFields), unitNanoMap[totalUnit], 1))(slots, totalUnit);
      }
      if (!relativeToSlots) {
        throw new RangeError("Missing relativeTo");
      }
      const [marker, caledarOps, timeZoneOps] = createMarkerSystem(getCalendarOps, getTimeZoneOps, relativeToSlots), markerToEpochNano = createMarkerToEpochNano(timeZoneOps), moveMarker = createMoveMarker(caledarOps, timeZoneOps);
      return ((durationFields, endEpochNano, totalUnit, marker, markerToEpochNano, moveMarker) => {
        const sign = computeDurationSign(durationFields), [epochNano0, epochNano1] = clampRelativeDuration(clearDurationFields(totalUnit, durationFields), totalUnit, sign, marker, markerToEpochNano, moveMarker), frac = computeEpochNanoFrac(endEpochNano, epochNano0, epochNano1);
        return durationFields[durationFieldNamesAsc[totalUnit]] + frac * sign;
      })(...spanDuration(slots, totalUnit, marker, markerToEpochNano, moveMarker, createDiffMarkers(caledarOps, timeZoneOps)), totalUnit, marker, markerToEpochNano, moveMarker);
    })(refinePublicRelativeTo, createDiffOps, createTimeZoneOps, slots, options),
    toLocaleString(slots, locales, options) {
    ...
    compare: (durationArg0, durationArg1, options) => ((refineRelativeTo, getCalendarOps, getTimeZoneOps, durationSlots0, durationSlots1, options) => {
      ...
      const [marker, calendarOps, timeZoneOps] = createMarkerSystem(getCalendarOps, getTimeZoneOps, relativeToSlots), markerToEpochNano = createMarkerToEpochNano(timeZoneOps), moveMarker = createMoveMarker(calendarOps, timeZoneOps);
      return compareBigNanos(markerToEpochNano(moveMarker(marker, durationSlots0)), markerToEpochNano(moveMarker(marker, durationSlots1)));
